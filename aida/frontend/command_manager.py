...
from aida.frontend.commands.base import CommandExecutor, CommandResult
...
class CommandManager(QObject):
    ...
    def active_command(self) -> str | None:
        return self._active_executor.task_name if self._active_executor else None
    ...
    def execute(self, command: RoutedCommand) -> bool:
        executor = self._registry.get(command.command_type)
        if executor is None:
            self._history.add_system("Recognized command has no registered executor.")
            ...
            return False
        if self.is_running:
            self._history.add_system("Another command is already running.")
            return False
        if self._task_manager.is_running(executor.task_name):
            self._history.add_system(f"{executor.task_name.upper()} is already running.")
            return False
        self._active_executor = executor
        self._status_manager.set(AIDAStatus.ANALYZING)
        self._history.add_system(executor.start_message)
        ...
        if not started:
            self._history.add_system(f"{executor.task_name.upper()} could not be started.")
            self._status_manager.set(AIDAStatus.ERROR)
            self.command_status_changed.emit(executor.category.name, "ERROR")
            self._active_executor = None
            self._status_manager.set(AIDAStatus.STANDBY)
            self.input_enabled_requested.emit(True)
            return False
        self.command_status_changed.emit(executor.category.name, "RUNNING")
    ...
    def _handle_result(self, result: object) -> None:
        if not isinstance(result, CommandResult):
            self._history.add_system("Command returned an unexpected result type.")
            self._status_manager.set(AIDAStatus.ERROR)
            return
        self._history.add_aida(result.transcript_text)
        if result.ui_action:
            self.ui_action_requested.emit(result.ui_action)
    ...
    def _handle_error(self, error_message: str) -> None:
        ...
        task_name = executor.task_name if executor else "command"
        self._history.add_system(f"{task_name.upper()} failed: {error_message}")
        self._status_manager.set(AIDAStatus.ERROR)
        if executor:
            self.command_status_changed.emit(executor.category.name, "ERROR")
    ...
    def _handle_finished(self) -> None:
        ...
        task_name = executor.task_name if executor else "command"
        if executor:
            self.command_status_changed.emit(executor.category.name, "IDLE")
