...
from aida.frontend.technomancer_ui import TechnomancerFrontendBridge
...
def main() -> int:
    ...
    technomancer_ui = TechnomancerFrontendBridge(
        window=window,
        overlay=overlay,
        command_manager=command_manager,
    )

    for proactive_message in command_registry.technomancer.proactive_messages(limit=2):
        history.add_aida(proactive_message)
    ...
    overlay_visibility_timer.timeout.connect(sync_overlay_visibility)
