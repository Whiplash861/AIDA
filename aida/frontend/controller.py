...
class AIDAController:
    ...
    def _handle_task_failed(
        ...
        self.window.report_task_failed(
            task_name,
            error_message
        )
    ...
    def _connect_components(self) -> None:
        self.window.set_submit_handler(
    ...
    def handle_user_message(self, text: str) -> None:
        ...
        if self.status_manager.current is not AIDAStatus.STANDBY:
            return
        ...
        if routed_command is not None:
            started = self.command_manager.execute(
                routed_command
            )
    ...
    def _handle_command_status_changed(
        ...
        if category in {
            ...
            self.window.set_diagnostics_status(
                status
            )

        elif category == "MEMORY":
            self.window.set_memory_status(
                status
            )
    ...
    def shutdown(self) -> None:
        ...
        self.task_manager.wait_for_done(
            timeout_ms=5000
        )