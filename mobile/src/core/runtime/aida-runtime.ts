...
export async function submitLocalDirective(message: string): Promise<string> {
  ...
  await addActivity(
    'AIDA',
    `Reasoning response generated by ${response.provider} (${response.mode}).`,
    'info',
    'mobile.reasoning',
  );
    ...
    snapshot = {
      ...
      capabilities: [
        ...
        },
        {
          ...
          state: 'supported',
          detail: `The runtime currently uses ${MOBILE_REASONING.currentProviderId()} and can be switched to an authenticated gateway without changing the frontend.`,
        },
        {
          id: 'reasoning.gateway',
          label: 'AIDA reasoning gateway',
          state: 'staged',
          detail: 'Secure independent cloud reasoning will be connected without embedding Azure credentials in the application.',
        },
        {
          id: 'voice.mobile',
