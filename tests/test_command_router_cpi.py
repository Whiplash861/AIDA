...
def test_router_understands_surface_synonyms():
    ...
    assert (
        router.route("perform a surface-level scan").command_type
        is CommandType.SECURITY_SURFACE_SCAN
    )
    assert (
        router.route("Start a surface scan").command_type
        is CommandType.SECURITY_SURFACE_SCAN
    )
    assert (
        router.route("Initiate low-level scan").command_type
        is CommandType.SECURITY_SURFACE_SCAN
    )
...
def test_router_maps_stand_down_revocation_and_confirmation():
    router = CommandRouter()
    request = router.route(
        r'revoke stand down for "C:\Program Files\Example\example.exe"'
    )
    confirmation = router.route("confirm stand down revocation")

    assert request.command_type is CommandType.STAND_DOWN_REVOKE_REQUEST
    assert request.target_path == r"C:\Program Files\Example\example.exe"
    assert confirmation.command_type is CommandType.STAND_DOWN_REVOKE_CONFIRM


def test_router_maps_read_only_observation_check():
    command = CommandRouter().route("observe security posture")

    assert command.command_type is CommandType.AUTONOMY_OBSERVE_SECURITY
