...
def test_unqualified_generic_scan_still_requires_clarification(resolver):
    result = resolver.resolve("scan the computer")
...
def test_full_and_deep(resolver):
    assert (
        resolver.resolve("perform a full system sweep").resolved.intent_id
        == "security.scan.full"
    )
...
def test_memory_and_application_slots(resolver):
    add = resolver.resolve(
        "remember that browser passwords must never be cleared"
    ).resolved
...
def test_confirmation_phrase_is_exact_intent(resolver):
    assert (
        resolver.resolve("confirm scan cancellation").resolved.intent_id
        == "security.scan.cancel.confirm"
    )
...
def test_application_recovery_intents(
    resolver,
    phrase,
    intent_id,
    application,
